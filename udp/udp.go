...
func (cli *UDPClient) Exchange(request []byte, serverIp net.IP, serverPort uint32, response []byte) ([]byte, error) {
	conn, err := net.DialUDP("udp", nil, &net.UDPAddr{IP: serverIp, Port: int(serverPort)})
	if err != nil {
		return nil, fmt.Errorf("dial udp with addr %s and port %d failed: %s", serverIp, serverPort, err.Error())
	}

	defer conn.Close()

	if _, err := conn.Write(request); err != nil {
		return nil, fmt.Errorf("write udp request to addr %s and port %d failed: %s", serverIp, serverPort, err.Error())
	}

	conn.SetReadDeadline(time.Now().Add(cli.timeout))
	n, err := conn.Read(response)
	if err != nil {
		return nil, fmt.Errorf("read udp respsonse from addr %s and port %d failed: %s", serverIp, serverPort, err.Error())
	}
